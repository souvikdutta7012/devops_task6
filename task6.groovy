...
job("job2") {
  ...
  steps {
      shell("if sudo kubectl get pods | grep mypod; then sudo kubectl delete pods --all; fi;if sudo kubectl get service | grep myservice ; then sudo kubectl delete service --all; fi;if sudo kubectl get pvc | grep mywebpvc; then sudo kubectl delete pvc --all; fi;sudo kubectl create -f /root/devopstask6/mypvc.yml;sudo kubectl create -f /root/devopstask6/deploy.yml;sudo kubectl create -f /root/devopstask6/service.yml;sudo kubectl get all;") 
  }
}
...
job("job3") {
  ...
  steps {
    shell("sudo cd /root/devopstask6")
